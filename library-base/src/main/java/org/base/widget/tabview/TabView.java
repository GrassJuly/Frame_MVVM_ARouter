package org.base.widget.tabview;
...
public class TabView extends RelativeLayout {
    ...
    /**
     * the view background color , default is grey
     */
    ...
    private void initDefaultAttrs(Context context) {
        ...
        mImageViewHeight = DensityUtils.dip2dp(context, 20);
    ...
    private void initView(Context context) {
        ...
        LayoutParams viewParams = new LayoutParams(RMP, DensityUtils.dp2px(context, 1));
        ...
        if (mTabViewGravity == Gravity.BOTTOM) {
            ...
            viewParams.addRule(RelativeLayout.ABOVE, R.id.tabview_id);
            fragmentContainerParams.addRule(RelativeLayout.ABOVE, R.id.view);
            ...
            viewParams.addRule(RelativeLayout.RIGHT_OF, R.id.tabview_id);
            fragmentContainerParams.addRule(RelativeLayout.RIGHT_OF, R.id.view);
        } else if (mTabViewGravity == Gravity.TOP) {
            tabviewParams = new LayoutParams(RMP, mTabViewHeight);
            tabview.setOrientation(LinearLayout.HORIZONTAL);
            tabviewParams.addRule(RelativeLayout.ALIGN_PARENT_TOP);
            viewParams.addRule(RelativeLayout.BELOW, R.id.tabview_id);
            fragmentContainerParams.addRule(RelativeLayout.BELOW, R.id.view);
            ...
            fragmentContainerParams.addRule(RelativeLayout.LEFT_OF, R.id.view);
        }
        ...
        for (int i = 0; i < mTabViewChildList.size(); i++) {
            ...
            tabChild.setOnClickListener(new OnClickListener() {
                ...
                public void onClick(View view) {
                    ...
                    showOrHide();
                    if (listener != null) {
                        listener.onTabChildClick(mFragments[currentPosition], currentPosition, imageview, textview);
                    }
                }
            });
        }
    ...
    public void setOnTabChildClickListener(OnTabChildClickListener l) {
        listener = l;
        if (listener != null)
            listener.onTabChildClick(mFragments[mTabViewDefaultPosition], mTabViewDefaultPosition);
    }

    public interface OnTabChildClickListener {
        void onTabChildClick(Fragment fragment, int position);

        void onTabChildClick(Fragment fragment, int position, ImageView imageView, TextView textView);
    }
    ...
    public void setTextViewSize(int size) {
        this.mTextViewSize = DensityUtils.sp2px(getContext(), size);
    }
